## I create two functions: one creates a special object that can store its inverse 
## and the other just calculates the inverse of a square matrix taking the previous
## special object as input and verifying that the inverse hasn't been calculated before.   
...
cacheSolve <- function(x, ...) {
        s <- x$getsolve()               # 1.first check if an inverse matrix 
                                        # has already been calculated. If that's so, 
                                        # print a message on screen, return the 
                                        # value and exit the function. 
        if(!is.null(s)){
                message("Getting cached data...")       
                return(s)
        }
        input <- x$get()                # 2.if there is no cached inverse, then
                                        # get the matrix and store it in an object called "input"
        s <- solve(input, ...)          # 3.solve the matrix and store the result in "s"
        x$setsolve(s)                   # 4.set the "setsolve" element in the "x" list 
                                        # to the inverse matrix, so it is cached.
        s                               # 5.finally, return the newly calculated inverse matrix
}

############    Testing....   ###########
# These lines are just for testing purposes for myself, I just leave them thre, commented
